...
class _JSFUniformOrderStat3D(_GPUBackend):
    ...
    def heat(self, log_du):
        ...
        # using matrix algebra
        # initialise probabilities to 0
        t_P_total = torch.zeros((log_du.shape[0],),
            dtype=torch.float32 if self.precision == 'float' else torch.float64, device=t_device)

        # distribute matrix entries across processes: TODO abstract into ht.fromiter()
        combs_gshape = (self.num_iterations, self.d)
        offset, combs_lshape, combs_slice = log_du.comm.chunk(combs_gshape, split=0)
        t_lcombs = torch.empty(combs_lshape, dtype=torch.int64, device=t_device)
...
def _pmat_neighbors_ht(mat, filter_shape, n_largest):
    """
    Build the 3D matrix `L` of largest neighbors of elements in a 2D matrix
    `mat`.
    ...
    # by the diagonal
    if symmetric:
        bin_range_y = range(l, N_bin_y - l + 1) if mat.comm.rank==0 else range(N_bin_y - l + 1)
    else:
...
def _intersection_matrix_ht(spiketrains, spiketrains_y, bin_size, t_start_x,
    ...
    for r in range(size):
        ...
        imat.larray[local_row_slice, column_slice] = torch.tensor(
...
class ASSET(object):
    ...
    def intersection_matrix_ht(self, normalization=None):
        """
        Generates the intersection matrix from a list of spike trains.
        ...
        """
        imat = _intersection_matrix_ht(self.spiketrains_i, self.spiketrains_j,
                                    self.bin_size,
                                    self.t_start_i, self.t_start_j,
                                    self.t_stop_i, self.t_stop_j,
                                    normalization=normalization)
